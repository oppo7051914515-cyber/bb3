...
<body class="bg-slate-50 text-slate-800 min-h-screen flex flex-col selection:bg-blue-500 selection:text-white">
            ...
            <div class="flex items-center space-x-3 cursor-pointer" onclick="switchTab('dashboard')">
                ...
                <div>
                    <h1 class="font-bold text-lg leading-tight flex items-center">
                        Smart Check-In
                        <span class="ml-2 text-[10px] bg-blue-500/20 text-blue-400 border border-blue-500/30 font-semibold px-2 py-0.5 rounded-full">v3.5 Fixed Sync</span>
                    </h1>
    ...
    <main class="flex-1 max-w-7xl w-full mx-auto p-4 md:p-6">
        ...
        <div id="student-view" class="hidden max-w-md mx-auto py-4">
            ...
            <div class="glass-card rounded-2xl shadow-xl p-6 text-center border-t-4 border-blue-600 relative overflow-hidden">
                ...
                <div id="student-state-loading" class="py-8 space-y-3">
                    ...
                    <h3 class="text-base font-bold text-slate-700">กำลังเชื่อมต่อคาบเรียน...</h3>
                    <p class="text-xs text-slate-400">กรุณารอสักครู่ ระบบกำลังดึงข้อมูลจาก Cloud</p>
                </div>

                <!-- State Container: Closed / Error -->
                <div id="student-state-closed" class="hidden py-4">
                    ...
                    <h2 class="text-xl font-bold text-slate-800" id="closed-title">คาบเรียนนี้ถูกปิดแล้ว</h2>
                    <p class="text-xs text-slate-500 mt-2 mb-4" id="closed-desc">คุณครูได้ทำการปิดคาบเรียนนี้แล้ว ไม่สามารถเช็คชื่อเพิ่มได้</p>
                ...
                <!-- State Container: Active Form -->
                <div id="student-state-active" class="hidden">
                    <div class="w-16 h-16 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-4 text-2xl shadow-inner border border-blue-100">
                        <i class="fa-solid fa-user-check"></i>
                    </div>
                    <h2 class="text-xl font-bold text-slate-800" id="student-class-title">กำลังดึงข้อมูล...</h2>
        ...
        <!-- OTHER TABS (Classes, Students, Reports) -->
    ...
    <script>
        ...
        async function toggleClassSession() {
            ...
            if (!currentSessionId) {
                ...
                if (!classId) {
                    Swal.fire({ icon: 'warning', title: 'โปรดเลือกห้องเรียน' });
                    return;
                }
                ...
                try {
                    ...
                    Swal.fire({ icon: 'error', title: 'เชื่อมต่อ Cloud ล้มเหลว', text: e.message });
        ...
        async function verifyTeacherOTP() {
            ...
            const student = Object.values(currentSessionData.students).find(s => s.otp === otpCode && s.status === 'pending');
        ...
        // Student Data Fetching & Handling
        async function fetchStudentSessionData(manualRefresh = false) {
            ...
            try {
                ...
                if (!data || data === null || data.active !== true) {
                    if (retryCount < 2) {
                        retryCount++;
                        return;
                    }
        ...
        // Classroom & Student Management Functions
        function addClass() {
